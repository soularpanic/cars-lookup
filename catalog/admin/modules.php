...
echo("DEBUG: module_key = " . $module_key . "<br/>");
var_dump($modules_installed);
  if ($dir = @dir($module_directory)) {
    while ($file = $dir->read()) {
      echo("DEBUG: reading file " . $file . "</br>");
...
<?php
		echo("DEBUG: HIHIHI" . sizeof($directory_array));

  $installed_modules = array();
  for ($i=0, $n=sizeof($directory_array); $i<$n; $i++) {
    $file = $directory_array[$i];
    echo("<br/>DEBUG: \$file = " . $file . "<br/>");
    echo("<br/>DEBUG: \$module_type = $module_type");
    include($module_language_directory . $language . '/modules/' . $module_type . '/' . $file);
    echo("<br/>DEBUG: marker alpha<br/>");
    echo("<br/>DEBUG: \$module_directory = $module_directory<br/>");
    include($module_directory . $file);
    echo("<br/>DEBUG: marker beta<br/>");

    $class = substr($file, 0, strrpos($file, '.'));
    echo("<br/>DEBUG: \$class = $class<br/>");
    if (tep_class_exists($class)) {
      $module = new $class;
      echo("<br/>DEBUG: \$module.check() = " . $module->check() . "<br/>");
      if ($module->check() > 0) {
	...
	echo("DEBUG: dumping \$module_info...");var_dump($module_info);
      ...
      if (isset($mInfo) && is_object($mInfo) && ($class == $mInfo->code) ) {
        if ($module->check() > 0) {
          ...
          echo '              <tr id="defaultSelected" class="dataTableRowSelected" onmouseover="rowOverEffect(this)" onm
ouseout="rowOutEffect(this)">' . "\n";
        }
      } else {
...
<?php
  ...
  switch ($action) {
	...
	echo("<br/>DEBUG: in new list block<br/>"); 
        if (isset($mInfo)) {
